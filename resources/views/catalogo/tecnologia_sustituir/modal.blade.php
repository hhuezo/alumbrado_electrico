<div class="modal fade fixed top-0 left-0 hidden w-full h-full outline-none overflow-x-hidden overflow-y-auto" aria-hidden="true" role="dialog" tabindex="-1" id="modal-delete-{{ $obj->id }}">
    ...
    <form method="POST" action="{{ route('tipo_luminaria.delete_tecnologia_sustituir',$PotenciaPromedio->id ) }}">
        @method('delete')
        @csrf
        <input type="hidden" name="tecnologia_sustituir_id_eliminar" value="{{$obj->id}}">
